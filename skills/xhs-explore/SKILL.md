...
**执行方式**：将泛化后的关键词用逗号分隔传给 `browse --keyword`，同时提供 `--filter` 筛选词：

```bash
python scripts/cli.py browse \
  --keyword "男女对立,找对象 180 有钱 过分吗,彩礼 到底该不该,A8 A9 择偶,女生消费观 争议" \
  --filter "男女,对立,找对象,彩礼,择偶,有钱,过分,消费观,女生,男生,婚姻,恋爱,单身,条件,标准,凭什么,嫁,娶,相亲,A8,A9,宾利" \
  --max-notes 15 \
  --max-per-keyword 4
```
...
`--filter` 参数接受逗号分隔的筛选词列表。浏览时会检查每张卡片的标题，**只有标题中包含至少一个筛选词的卡片才会被点击浏览**，其余跳过。

**AI 在生成搜索关键词时，必须同时生成 `--filter` 筛选词：**

1. **筛选词要比搜索词更宽泛**：搜索词用于精准搜索，筛选词用于排除明显不相关的推荐内容
2. **包含主题的核心概念词**：如"男女对立"主题应包含"男女、对立、择偶、彩礼、恋爱、婚姻"等
3. **包含相关的动作/情绪词**：如"过分、凭什么、争议、吐槽"等
4. **包含平台黑话/缩写**：如"A8、A9、npy、pua"等
5. **数量建议 10-25 个**：太少会漏掉相关内容，太多失去筛选意义

**示例：**

用户意图：`找挑动男女对立的帖子`
```
--filter "男女,对立,找对象,彩礼,择偶,有钱,过分,消费观,女生,男生,婚姻,恋爱,单身,条件,标准,凭什么,嫁,娶,相亲,A8,A9,宾利,pua,直男,女权,拜金"
```

用户意图：`看看大家怎么评价996`
```
--filter "996,加班,内卷,躺平,打工,离职,工作,上班,老板,公司,薪资,工资,猝死,摸鱼,辞职,福报,奋斗,压榨"
```
...
以模拟人类的节奏搜索并浏览笔记，支持多关键词泛化搜索 + 相关性预筛选：
...
python scripts/cli.py browse \
  ...
  --filter "青岛,海鲜,吃,团岛,市场,避坑,攻略,餐厅,大排档,蛤蜊,虾,蟹,鲍鱼,扇贝,啤酒" \
  --max-notes 15 \
  --max-per-keyword 4 \
  --max-time 600
```
...
- `--filter`：相关性筛选词（逗号分隔），卡片标题需包含至少一个词才会被浏览
